...
def main():
    ...
    bar.update(1)
    meta_id = sdb.query(AccountGitHubAccount.id) \
                 .filter(AccountGitHubAccount.account_id == args.account) \
                 .scalar()
    bar.update(1)
    user_ids = [
        r[0] for r in mdb.query(OrganizationMember.child_id)
                         .filter(OrganizationMember.acc_id == meta_id).all()
    ]
    bar.update(1)
    users = mdb.query(GitHubUser).filter(and_(GitHubUser.acc_id == meta_id,
