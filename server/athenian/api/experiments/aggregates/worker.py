...
async def refresh_aggregates(
        sdb_conn: str, mdb_conn: str, pdb_conn: str, adb_conn: str,
        accounts: List[int], cache_conn: Optional[str] = None,
    ...
    async with _get_db_conns(sdb_conn, mdb_conn, pdb_conn, adb_conn) as conns:
        ...
        async with _lock_enabled(adb):
            ...
            for account, repos in account_repos.items():
                ...
                log.info("Refreshing aggregates for account %d with %d repos...",
                         account, len(repos))
                ...
                await _checkpoint_aggregates_timestamps(
                    adb, prs_node_ids, checkpoint_timestamp)
...
async def _retrieve_and_insert_new_prs(
    ...
    await adb.execute_many(
        ...
        [PullRequestStatus(**{**pr, "account": account}).create_defaults().explode(
...
async def _retrieve_and_insert_new_prs_for_new_repos(
    ...
    current_repos = await adb.fetch_all(
        select([PullRequestStatus.repository_node_id,
                PullRequestStatus.repository_full_name], distinct=True))
    ...
    log.info("Retrieving new PRs for each new repository from %s...",
             table_fqdn(mdb, PullRequest))
    ...
    log.info("Inserting %d new PRs to %s...", len(prs_for_new_repos),
             table_fqdn(adb, PullRequestStatus))
    await adb.execute_many(
        insert(PullRequestStatus),
        [PullRequestStatus(**{**pr, "account": account}).create_defaults().explode(
...
async def _get_outdated_aggregation_prs(
    ...
    async def fetch_all_unfresh_prs() -> pd.DataFrame:
        log.info(
            ...
            table_fqdn(adb, PullRequestStatus), freshness_threshold_seconds,
        )
        # SELECT
        #     repository_node_id,
    ...
    async def fetch_all_release_setting() -> pd.DataFrame:
        log.info("Retrieving corresponding release settings from %s...",
        ...
        return await read_sql_query(
            ...
            sdb, ["repository_full_name", "last_release_settings_update_timestamp"],
        )

    # 3. fetch latest updated_at for each PR
    async def fetch_all_updated_at_prs(prs_node_ids: Set[str]) -> pd.DataFrame:
        log.info("Retrieving corresponding last updates from %s...",
                 table_fqdn(mdb, PullRequestStatus))
        return await read_sql_query(
            select([
                PullRequest.node_id,
    ...
    df = unfresh_prs\
        .join(updated_prs.set_index("node_id"), on="node_id")\
        .join(release_settings.set_index("repository_full_name"), on="repository_full_name")
    ...
    # Format return value
    grouped_outdated_prs = outdated_prs[["repository_full_name", "number"]].groupby(
        "repository_full_name")["number"].apply(list).reset_index()
...
def _eventify_prs(account: int, prs_data: List[PullRequestListItem],
                  releases_match_settings: ReleaseMatchSetting) -> List[PullRequestEvent]:
    ...
    def enrich_released_event(row, pr):
        ...
        row["release_count"] = 1
        row["lead_time"] = sum(int(pr.stage_timings[s].total_seconds())
                               if pr.stage_timings[s] else 0
    ...
    return [PullRequestEvent(**ev).create_defaults().explode(with_primary_keys=True)
            for ev in events]
...
async def _checkpoint_aggregates_timestamps(
    ...
    await adb.execute(
        update(PullRequestStatus).where(PullRequestStatus.node_id.in_(prs_node_ids))
...
def _parse_args():
    ...
    parser.add_argument("--metadata-db", required=True, dest="mdb_conn",
                        help="Metadata DB endpoint, e.g. postgresql://0.0.0.0:5432/metadata")
    parser.add_argument("--precomputed-db", required=True, dest="pdb_conn",
    ...
    parser.add_argument("--memcached", required=False, dest="cache_conn",
                        help="memcached address, e.g. 0.0.0.0:11211")
    parser.add_argument("--accounts", required=False, action="extend", dest="accounts",
                        help="Account to refresh the aggregates for")
    parser.add_argument("--create-tables", required=False, action="store_true",
                        dest="create_tables",
...
def main():
    ...
    asyncio.run(
        refresh_aggregates(
            args.sdb_conn, args.mdb_conn, args.pdb_conn, args.adb_conn,
            [int(acc) for acc in args.accounts], cache_conn=args.cache_conn,
        ),
        debug=args.debug,
    )
