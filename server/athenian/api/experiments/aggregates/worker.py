...
async def _retrieve_and_insert_new_prs_for_new_repos(
    ...
    log.info(
        "Inserting %d new PRs to %s...", len(prs_for_new_repos), table_fqdn(adb, PullRequestStatus)
    )
    await adb.execute_many(
        insert(PullRequestStatus),
        [
...
def _eventify_prs(
    account: int, prs_data: List[PullRequestListItem], releases_match_settings: ReleaseMatchSetting
...
async def _checkpoint_aggregates_timestamps(
    adb: Database, prs_node_ids: List[str], checkpoint_timestamp: datetime
):
    log.info(
