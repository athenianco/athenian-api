                ...
                if group_props is not None:
                    ...
                    if df_col == Issue.labels.name:
                        if labels_matcher is None:
                            labels_matcher = _ArraysWhitelistMatcher(df_values)
                        matching = labels_matcher(filter_values)
                        not_maching_mask = np.ones(len(df_values), dtype=bool)
                        not_maching_mask[matching] = False
                        df_matches[not_maching_mask] = False
                    else:
