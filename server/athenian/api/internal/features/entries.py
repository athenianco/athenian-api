...
class MetricEntriesCalculator:
    ...
    @cached(
        ...
        key=lambda metrics, time_intervals, quantiles, lines, repositories, participants, labels, jiragroups, environments, exclude_inactive, release_settings, logical_settings, **_: (  # noqa
            ...
            labels,
            ",".join(str(j) for j in jiragroups),
    ...
    async def calc_pull_request_metrics_line_github(
        ...
        labels: LabelFilter,
        jiragroups: Sequence[JIRAFilter],
        ...
        :return: jiragroups x lines x repositories x participants x
        ...
        global_jira_filter = reduce(operator.or_, jiragroups)
        ...
        df_facts = await pr_facts_calc(
            ...
            prefixer,
            fresh,
            self._get_jira_entities_to_fetch(jiragroups, metrics),
            branches,
            default_branches,
        )

        jira_grouper = await self._jira_filters_prs_grouper(jiragroups)
