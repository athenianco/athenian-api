...
class PullRequestMiner:
    ...
    async def _mine_by_ids(
        ...
        if len(merged_unreleased_indexes):
            ...
            label_matches = np.flatnonzero(
                np.in1d(
                    dfs.labels.index.get_level_values(0).values, merged_unreleased_prs.index.values
                )
            )
            labels = {}
            for k, v in zip(
    ...
    async def _store_empty_pr_check_runs(
        cls, missed_empty_pr_ids: Iterable[int], account: int, pdb: Database
    ) -> None:
    ...
    def _check_participants_compatibility(
        cached_participants: PRParticipants, participants: PRParticipants
    ) -> bool:
    ...
    def _find_drop_by_jira(cls, dfs: PRDataFrames, jira: JIRAFilter) -> pd.Index:
        ...
        if jira.labels:
            ...
            left.append(
                find_left_prs_by_labels(pr_node_ids, df_labels_index, df_labels_names, jira.labels)
            )
        if jira.epics:
            left.append(
                jira_index.take(np.flatnonzero(dfs.jiras["epic"].isin(jira.epics).values)).unique()
            )
        if jira.issue_types:
            left.append(
    ...
    def _find_drop_by_inactive(
        cls, dfs: PRDataFrames, time_from: datetime, time_to: datetime
    ) -> pd.Index:
        activities = [
