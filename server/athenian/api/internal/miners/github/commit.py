...
from athenian.api.internal.miners.github.dag_accelerated import append_missing_heads, \
...
@cached(
    ...
    key=lambda prop, date_from, date_to, repos, with_author, with_committer,
    only_default_branch, **kwargs:  # noqa
    (
        prop.value,
        date_from.timestamp(),
        date_to.timestamp(),
        ",".join(sorted(repos)),
        ",".join(sorted(with_author))
        if with_author is not None and len(with_author) else "",
        ",".join(sorted(with_committer))
        if with_committer is not None and len(with_committer) else "",  # noqa
        "" if kwargs.get("columns") is None else ",".
        join(c.name for c in kwargs["columns"]),
        only_default_branch,
    ),
)
async def extract_commits(
        ...
        user_ids = dict(
            await mdb.fetch_all(
                select([User.login, User.node_id]).where(
                    and_(
                        User.login.in_(user_logins), User.acc_id.in_(meta_ids)
                    )
                )
            )
        )
    ...
    if columns is None:
        ...
        for col in (PushCommit.node_id, PushCommit.repository_full_name,
                    PushCommit.sha):
    ...
    if prop == FilterCommitsProperty.NO_PR_MERGES:
        ...
        commits_task = read_sql_query(
            select(cols_query).select_from(
                ...
                and_(NodePullRequestCommit.commit_id.is_(None), *sql_filters)
            ), mdb, cols_df
        )
    else:
    ...
    tasks = [
        ...
        fetch_repository_commits_from_scratch(
            repos, branch_miner, True, prefixer, account, meta_ids, mdb, pdb,
            cache
        ),
    ]
    commits, (dags, branches, default_branches) = await gather(
        *tasks, op="extract_commits/fetch"
    )
    candidates_count = len(commits)
    if only_default_branch:
        commits = _take_commits_in_default_branches(
            commits, dags, branches, default_branches
        )
        log.info(
            "Removed side branch commits: %d / %d", len(commits),
            candidates_count
        )
    else:
        commits = _remove_force_push_dropped(commits, dags)
        log.info(
            "Removed force push dropped commits: %d / %d", len(commits),
            candidates_count
        )
    for number_prop in (PushCommit.additions, PushCommit.deletions,
                        PushCommit.changed_files):
        try:
            number_col = commits[number_prop.name]
        except KeyError:
            continue
        nans = commits[PushCommit.node_id.name].take(
            np.where(number_col.isna())[0]
        )
        if not nans.empty:
            log.error(
                "[DEV-546] Commits have NULL in %s: %s", number_prop.name,
                ", ".join(nans)
            )
    return commits
...
def _take_commits_in_default_branches(
    ...
    branch_repos_names = np.char.add(
        np.char.add(branch_repos, "|"), branch_names
    )
    default_repos_names = np.array(
        [f"{k}|{v}" for k, v in default_branches.items()], dtype="U"
    )
    default_mask = np.in1d(
        branch_repos_names, default_repos_names, assume_unique=True
    )
    branch_repos = branch_repos[default_mask]
    branch_hashes = branches[Branch.commit_sha.name
                             ].values[default_mask].astype("S")
    ...
    commit_repos, commit_repo_indexes = np.unique(
        commits[PushCommit.repository_full_name.name].values.astype("U"),
        return_inverse=True
    )
    commit_repos_in_branches_mask = np.in1d(
        commit_repos, branch_repos, assume_unique=True
    )
    branch_repos_in_commits_mask = np.in1d(
        branch_repos, commit_repos, assume_unique=True
    )
    ...
    for repo, head_sha, commit_repo_index in zip(
            branch_repos, branch_hashes,
            np.nonzero(commit_repos_in_branches_mask)[0]):
        ...
        default_branch_hashes = extract_subdag(
            *dags[repo], np.array([head_sha])
        )[0]
        accessible_indexes.append(
            repo_indexes[np.in1d(
                repo_hashes, default_branch_hashes, assume_unique=True
            )]
        )
...
def _remove_force_push_dropped(
    commits: pd.DataFrame, dags: Dict[str, DAG]
) -> pd.DataFrame:
    if commits.empty:
        return commits
    ...
    for i, repo in enumerate(repos_order):
        ...
        accessible_indexes.append(
            repo_indexes[
                np.in1d(repo_hashes, dags[repo][0], assume_unique=True)]
        )
    accessible_indexes = np.sort(np.concatenate(accessible_indexes))
    return commits.take(accessible_indexes)
...
@cached(
    ...
    key=lambda repos, branches, columns, prune, **_: (
        ...
        ",".join(
            np.sort(
                branches[columns[0] if isinstance(columns[0], str) else
...
async def fetch_repository_commits(
    ...
    columns: Tuple[Union[str, InstrumentedAttribute],
                   ...
                   Union[str,
                         InstrumentedAttribute], Union[str,
    ...
    missed_counter = 0
    repo_heads = {}
    sha_col, id_col, dt_col, repo_col = (
    ...
    unique_repos, index_map, counts = np.unique(
        df_repos, return_inverse=True, return_counts=True
    )
    ...
    for i, repo in enumerate(unique_repos):
        ...
        if len(hashes) > 0:
            ...
            missed_heads = required_heads[
                missed_mask]  # these hashes do not exist in the p-DAG
        else:
            missed_heads = required_heads
        if len(missed_heads) > 0:
            ...
            order = sorted(
                [(hash_to_dt[h], i) for i, h in enumerate(missed_heads)],
                reverse=True
            )
            ...
            tasks.append(
                _fetch_commit_history_dag(
                    hashes, vertexes, edges, missed_heads, missed_ids, repo,
                    meta_ids, mdb
                )
            )
        else:
            if prune:
                hashes, vertexes, edges = extract_subdag(
                    hashes, vertexes, edges, required_heads
                )
            result[repo] = hashes, vertexes, edges
    # traverse commits starting from the missing branch heads
    add_pdb_hits(
        pdb, "fetch_repository_commits",
        len(branches) - missed_counter
    )
    ...
    if tasks:
        ...
        for repo, hashes, vertexes, edges in new_dags:
            ...
            if prune:
                hashes, vertexes, edges = extract_subdag(
                    hashes, vertexes, edges, repo_heads[repo]
                )
            result[repo] = hashes, vertexes, edges
        if pdb.url.dialect == "postgresql":
            ...
            sql = sql.on_conflict_do_update(
                ...
                set_={
                    ...
                    GitHubCommitHistory.updated_at.name:
            ...
            raise AssertionError(
                "Unsupported database dialect: %s" % pdb.url.dialect
            )
...
async def fetch_repository_commits_from_scratch(
    ...
    dags = await fetch_repository_commits_no_branch_dates(
        pdags, branches, BRANCH_FETCH_COMMITS_COLUMNS, prune, account,
...
async def fetch_precomputed_commit_history_dags(
    ...
    format_version = ghrc.__table__.columns[ghrc.format_version.key
                                            ].default.arg
...
def _empty_dag() -> DAG:
    return np.array([], dtype="S40"), np.array([
        0
    ], dtype=np.uint32), np.array([], dtype=np.uint32)
...
async def _fetch_commit_history_dag(
    ...
    log = logging.getLogger(
        "%s._fetch_commit_history_dag" % metadata.__package__
    )
    ...
    if len(stop_heads) > 0:
        if len(stop_heads) > max_stop_heads:
            ...
            rows = await mdb.fetch_all(
                select([NodeCommit.oid]).where(
                    and_(
                        NodeCommit.oid.in_(stop_heads.astype("U40")),
            ...
            stop_heads = np.fromiter(
                (r[0] for r in rows), dtype="S40", count=len(rows)
            )
        ...
        with sentry_sdk.start_span(op="partition_dag", description="%d %d" %
                                   (len(hashes), len(partition_seeds))):
            stop_hashes = partition_dag(
                hashes, vertexes, edges, partition_seeds
            ).astype("U40")
    else:
        stop_hashes = []
    batch_size = 20
    while len(head_hashes) > 0:
        ...
        if not validate_edges_integrity(new_edges):
            log.warning(
                ...
                if len(new_edges) < 100 else f"<{len(new_edges)} new edges>"
            )
            new_edges = []
        else:
            ...
            if len(hashes) and len(orphans := find_orphans(new_edges, hashes)):
                ...
                if latest_commit is None:
                    log.error(
                        "failed to fetch committed_date of %s",
                        orphans.tolist()
                    )
                    new_edges = []
                else:
                    ...
                    if datetime.now(timezone.utc) - latest_commit < timedelta(
                            days=1, hours=6):
                        log.warning(
        ...
        if len(head_hashes) > 0 and len(hashes) > 0:
            collateral = np.flatnonzero(
                hashes[searchsorted_inrange(hashes, head_hashes)] ==
                head_hashes
            )
            if len(collateral) > 0:
...
async def _fetch_commit_history_edges(
    ...
    assert isinstance(
        mdb, Database
    ), "fetch_all() must be patched to avoid re-wrapping"
    if mdb.url.dialect == "sqlite":
        rq = "`"
        tq = '"'
    else:
...
async def fetch_dags_with_commits(
    ...
    dags = await fetch_repository_commits(
        pdags, commits, COMMIT_FETCH_COMMITS_COLUMNS, prune, account, meta_ids,
        mdb, pdb, cache
    )
    return dags, commits
...
async def _fetch_commits_for_dags(
    ...
    return await read_sql_query(
        union_all(*queries), mdb, COMMIT_FETCH_COMMITS_COLUMNS
    )
