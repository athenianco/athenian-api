...
class ReleaseToPullRequestMapper:
    ...
    async def _find_releases_for_matching_prs(
        ...
        Dict[str, ReleaseMatch], pd.DataFrame, pd.DataFrame, ReleaseSettings, Dict[str, DAG]
    ]:
        """
    ...
    async def _fetch_repository_first_commit_dates(
        ...
        rows = await pdb.fetch_all(
            select(
                [GitHubRepository.repository_full_name, GitHubRepository.first_commit.label("min")]
            ).where(
                and_(
