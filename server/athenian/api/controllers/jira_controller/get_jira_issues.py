...
async def _fetch_comments(
    ...
    author_display_names = comments[Comment.author_display_name.name]
    issue_ids = comments[Comment.issue_id.name]
    createds = comments[Comment.created.name]
    ...
    for indexes in comments.groupby("issue_id"):
        ...
        comments_map[issue_id] = [
            WebJIRAComment(
                author=author_display_names[idx], created=createds[idx], rendered_body=bodies[idx],
            )
            for idx in indexes
        ]
    return comments_map
