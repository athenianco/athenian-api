...
async def _epic_flow(
    ...
    if JIRAFilterReturn.PRIORITIES in return_:
        priority_ids = np.unique(
            np.concatenate(
                [epics_df[Issue.priority_id.name].values, children_columns[Issue.priority_id.name]]
            )
        )
    else:
        priority_ids = []
    if JIRAFilterReturn.STATUSES in return_:
        ...
        status_ids = np.unique(
            np.concatenate(
                [epics_df[Issue.status_id.name].values, children_columns[Issue.status_id.name]]
            )
        )
        status_project_map = defaultdict(set)
        for status_id, project_id in chain(
            zip(epics_df[Issue.status_id.name].values, epics_df[Issue.project_id.name].values),
            zip(children_columns[Issue.status_id.name], children_columns[Issue.project_id.name]),
        ):
...
async def _issue_flow(
    ...
    if JIRAFilterReturn.STATUSES in return_:
        ...
        for status_id, project_id in zip(
            issues[Issue.status_id.name].values, issues[Issue.project_id.name].values
        ):
            status_project_map[status_id].add(project_id)
    else:
        statuses = []
    ...
    if JIRAFilterReturn.ISSUE_TYPES in return_ or JIRAFilterReturn.ISSUE_BODIES in return_:
        ...
        for project_id, issue_type_id in zip(
            issues[Issue.project_id.name].values, issues[Issue.type_id.name].values
        ):
    ...
    async def extract_labels():
        if JIRAFilterReturn.LABELS in return_:
            ...
            for updated, issue_labels in zip(
                issues[Issue.updated.name], issues[Issue.labels.name].values
            ):
    ...
    async def _fetch_prs() -> Tuple[
        Optional[Dict[str, WebPullRequest]], Optional[Dict[str, Deployment]]
    ]:
    ...
    if JIRAFilterReturn.LABELS in return_:
        for updated, issue_components in zip(
            issues[Issue.updated.name], issues[Issue.components.name].values
        ):
...
async def _fetch_statuses(
    ...
    return [
        JIRAStatus(
            name=row[Status.name.name], stage=row[Status.category_name.name], project=project
        )
        for row in rows
...
async def _calc_jira_entry(
    ...
    Tuple[JIRAMetricsRequest, List[List[datetime]], timedelta, np.ndarray, np.ndarray],
    Tuple[JIRAHistogramsRequest, Dict[HistogramParameters, List[str]], np.ndarray],
]:
    try:
        ...
        chain.from_iterable(
            (
                ...
                for granularity, ts, ts_values in zip(
                    filt.granularities, time_intervals, group_metric_values
                )
            )
