...
def make_release_participants_grouper(participants: List[ReleaseParticipants],
                                      ) -> Optional[Callable[[pd.DataFrame], List[np.ndarray]]]:
    """Create the grouping function by release participants."""
    if len(participants) < 2:
        return None

    def group_releases_by_participants(df: pd.DataFrame) -> List[np.ndarray]:
        indexes = []
    ...
    return group_releases_by_participants
