...
async def resolve_metrics_current_values(
    obj: Any, info: GraphQLResolveInfo, accountId: int, params: Mapping[str, Any],
) -> Any:
    ...
    team_rows, meta_ids = await gather(
        fetch_teams_recursively(
            accountId,
            sdb,
            select_entities=(Team.id, Team.members, Team.parent_id),
...
def _jirafy_teams(
    teams: Iterable[Collection[int]], jira_map: Mapping[int, str],
...
async def calculate_team_metrics(
    ...
    if pr_metrics:
        pr_participants = [
            {PRParticipationKind.AUTHOR: team} for team in _loginify_teams(team_members, prefixer)
        ]
        tasks.append(
...
def _build_metrics_response(
    nodes: Dict[int, Dict[str, Any]], triaged: Dict[str, Dict[int, object]], root_team_id: int,
) -> List[MetricValues]:
    return [
        MetricValues(metric, _build_metric_response(nodes, team_metric_values, root_team_id))
        for metric, team_metric_values in triaged.items()
    ]


def _build_metric_response(
    nodes: Dict[int, Dict[str, Any]], metric_values: Dict[int, object], team_id: int,
) -> TeamMetricValue:
    return TeamMetricValue(
        team_id=team_id,
        value=MetricValue(metric_values[team_id]),
        children=[
            _build_metric_response(nodes, metric_values, child)
