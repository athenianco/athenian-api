...
async def delete_goal(
    account_id: int, goal_id: int, sdb_conn: Connection
) -> None:
    ...
    if await sdb_conn.fetch_val(select_stmt) == 0:
        raise GoalMutationError(
            f"Goal {goal_id} not found", HTTPStatus.NOT_FOUND
        )
...
async def delete_team_goals(
    ...
    assert team_ids
    assert conn_in_transaction(sdb_conn)
    await _validate_team_goal_deletions(
        account_id, goal_id, team_ids, sdb_conn
    )
...
async def assign_team_goals(
    ...
    await _validate_team_goal_assignments(
        account_id, goal_id, assignments, sdb_conn
    )
...
async def _validate_goal_creation_info(
    ...
    teams_stmt = sa.select([Team.id]).where(
        sa.and_(
            ...
            Team.owner_id == creation_info.goal.account_id
        ),
    )
    ...
    if missing_team_ids := team_ids - existing_team_ids:
        ...
        raise GoalMutationError(
            f"Some teamId-s don't exist or access denied: {missing_repr}",
...
async def _validate_team_goal_deletions(
    ...
    if missing := [team_id for team_id in team_ids if team_id not in found]:
        ...
        raise GoalMutationError(
            f"TeamGoal-s to remove not found for teams: {missing_repr}",
...
async def _validate_team_goal_assignments(
    ...
    if not goal_exists:
        raise GoalMutationError(
            f"Goal {goal_id} doesn't exist or access denied",
            HTTPStatus.NOT_FOUND,
        )

    teams_stmt = sa.select([Team.id]).where(
        sa.and_(
            Team.id.in_(a.team_id for a in assignments),
            Team.owner_id == account_id,
        ),
    )
    found_teams = set(r[0] for r in await sdb_conn.fetch_all(teams_stmt))
    if missing_teams := [a.team_id for a in assignments
                         if a.team_id not in found_teams]:
        missing_teams_repr = ",".join(map(str, missing_teams))
        raise GoalMutationError(
            f"Team-s don't exist or access denied: {missing_teams_repr}",
            HTTPStatus.NOT_FOUND,
        )
