...
async def update_goal_template_in_db(template_id, sdb: DatabaseLike, **values: Any) -> None:
...
def parse_goal_repositories(val: Optional[list[list]]) -> Optional[list[RepoIdentity]]:
    """Parse the raw value in DB repositories JSON column as list of `RepoIdentity`."""
    if val is None:
        return val
    return [RepoIdentity(repo_id, logical_name) for repo_id, logical_name in val]


def dump_goal_repositories(repo_idents: Optional[Sequence[RepoIdentity]]) -> Optional[list[list]]:
    """Dump the sequence of RepoIdentity-s in the format used by DB repositories JSON column."""
