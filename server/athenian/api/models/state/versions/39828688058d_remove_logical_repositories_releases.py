...
def upgrade():
    ...
    if op.get_bind().dialect == "postgresql":
        op.execute(
            "ALTER TABLE repository_sets "
            "ALTER COLUMN items "
            "SET DATA TYPE jsonb "
            "USING items::jsonb;"
        )
        op.execute(
            "ALTER TABLE teams ALTER COLUMN members SET DATA TYPE jsonb USING members::jsonb;"
        )
    with op.batch_alter_table("release_settings") as bop:
        bop.alter_column("repository", type_=sa.String())
        bop.alter_column(
            "branches", type_=sa.String(), nullable=False, server_default="{{default}}"
        )
...
def downgrade():
    if op.get_bind().dialect == "postgresql":
        op.execute(
            "ALTER TABLE repository_sets ALTER COLUMN items SET DATA TYPE json USING items::json;"
        )
        op.execute(
            "ALTER TABLE teams ALTER COLUMN members SET DATA TYPE json USING members::json;"
        )
    with op.batch_alter_table("logical_repositories") as bop:
