...
class Team(Base):
    ...
    __tablename__ = "teams"
    __table_args__ = (
        sa.UniqueConstraint("owner_id", "name", name="uc_owner_name"),
...
def upgrade():
    ...
    res = conn.execute(
        sa.select(Team.owner_id, Team.id).where(Team.parent_id.is_(None)).order_by(Team.owner_id),
    )
    ...
    for account, rows in groupby(res.fetchall(), itemgetter(0)):
        ...
        stmt = (
            sa.update(Team)
            .where(sa.and_(Team.owner_id == account, Team.id.in_(first_level_team_ids)))
...
def downgrade():
    ...
    for account, root_team_rows in groupby(res.fetchall(), itemgetter(0)):
        ...
        stmt = (
            sa.update(Team)
            .where(sa.and_(Team.owner_id == account, Team.parent_id == root_team_ids[0]))
