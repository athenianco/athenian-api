async def test_cors_headers(client, headers):
    response = await client.request(method="GET", path="/v1/reposet/1", headers=headers, json={})
    assert set(response.headers["Access-Control-Expose-Headers"].split(",")) == {
        "X-Performance-DB",
        "X-Backend-Server",
        "X-Performance-Precomputed-Hits",
...
async def test_cors_cache(client, headers, client_cache):
    ...
    assert set(response.headers["Access-Control-Expose-Headers"].split(",")) == {
        "X-Performance-DB",
        "X-Backend-Server",
        "X-Performance-Cache-Ignores",
        "X-Performance-Cache-Hits",
        "X-Performance-Cache-Misses",
        "X-Performance-Precomputed-Hits",
        "X-Performance-Precomputed-Misses",
        "Content-Length",
        "Server",
