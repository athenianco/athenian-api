...
def sample_deployments():
    ...
    labels = pd.DataFrame({
        ...
        "labels": [
            pd.DataFrame({DeployedLabel.key.name: ["first", "second"],
                          DeployedLabel.value.name: ["alpha", "alpha"]}),
            pd.DataFrame({DeployedLabel.key.name: ["first", "third"],
                          DeployedLabel.value.name: ["alpha", "beta"]}),
            pd.DataFrame({DeployedLabel.key.name: ["first", "second"],
                          DeployedLabel.value.name: ["gamma", "alpha"]}),
            pd.DataFrame({DeployedLabel.key.name: ["first"],
                          DeployedLabel.value.name: ["gamma"]}),
        ],
    })
...
async def test_split_logical_deployed_components_title_one(sample_deployments):
    ...
    assert_frame_equal(split_components, pd.DataFrame({
...
async def test_split_logical_deployed_components_title_two(sample_deployments):
    ...
    split_components = split_logical_deployed_components(
        notifications, labels, components, ["src-d/go-git/alpha", "src-d/go-git/beta"],
        logical_settings,
    )
    split_components.reset_index(inplace=True)
    assert_frame_equal(split_components, pd.DataFrame({
