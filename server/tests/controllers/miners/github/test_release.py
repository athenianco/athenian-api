...
async def test_map_releases_to_prs_early_merges(
    ...
    prs, releases, matched_bys, dag = await map_releases_to_prs(
        ...
        [], [], JIRAFilter.empty(),
        release_match_setting_tag, None, None, mdb, pdb, None)
    assert len(prs) == 60
...
async def test_map_releases_to_prs_smoke(
    ...
    for _ in range(2):
        prs, releases, matched_bys, dag = await map_releases_to_prs(
            ...
            [], [], JIRAFilter.empty(),
            release_match_setting_tag, None, None, mdb, pdb, cache)
        await wait_deferred()
        assert len(prs) == 7
...
async def test_map_releases_to_prs_no_truncate(
    ...
    prs, releases, matched_bys, _ = await map_releases_to_prs(
        ...
        [], [], JIRAFilter.empty(),
        release_match_setting_tag, None, None, mdb, pdb, None, truncate=False)
    assert len(prs) == 8
...
async def test_map_releases_to_prs_empty(
    ...
    prs, releases, matched_bys, _ = await map_releases_to_prs(
        ...
        datetime(year=2019, month=12, day=2, tzinfo=timezone.utc),
        [], [], JIRAFilter.empty(), release_match_setting_tag, None, None, mdb, pdb, cache)
    await wait_deferred()
    assert prs.empty
...
async def test_load_releases_tag_or_branch_dates(
        branches, default_branches, release_match_setting_tag, mdb, pdb, cache, time_from, n,
        pretag):
...
async def test__find_dead_merged_prs_no_branches(mdb, pdb, dag):
    ...
    dags = dag.copy()
    dags["xxx"] = _empty_dag()
    dead_prs = await _find_dead_merged_prs(prs, dags, branches, (6366825,), mdb, pdb, None)
...
async def test_mine_releases_full_span(mdb, pdb, release_match_setting_tag):
    ...
    releases, avatars, matched_bys = await mine_releases(
        (6366825,), ["src-d/go-git"], {}, None, {}, time_from, time_to, JIRAFilter.empty(),
...
async def test_mine_releases_precomputed_smoke(
    ...
    assert len(avatars) == 124
    releases, avatars, _ = await mine_releases(
        (6366825,), ["src-d/go-git"], {}, None, {}, time_to, time_to, JIRAFilter.empty(),
        release_match_setting_tag, mdb, pdb, None)
    assert len(releases) == 0
    releases, avatars, _ = await mine_releases(
        (6366825,), ["src-d/go-git"], {}, branches, default_branches, time_from, time_to,
        JIRAFilter.empty(), release_match_setting_branch, mdb, pdb, None)
...
async def test_mine_releases_jira(mdb, pdb, release_match_setting_tag, cache):
    ...
    releases, avatars, _ = await mine_releases(
        (6366825,), ["src-d/go-git"], {}, None, {}, time_from, time_to,
...
async def test_precomputed_release_timespans(pdb):
    ...
    await _store_precomputed_release_match_spans(
        mg1, {"src-d/go-git": ReleaseMatch.tag}, time_from, time_to, pdb)
    mg2 = {ReleaseMatch.branch: {"master": ["src-d/go-git"]}}
    await _store_precomputed_release_match_spans(
        mg2, {"src-d/go-git": ReleaseMatch.branch}, time_from, time_to, pdb)
    await _store_precomputed_release_match_spans(
