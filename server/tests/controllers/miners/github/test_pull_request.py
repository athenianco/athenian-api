...
async def test_pr_miner_jira_filter(
    ...
    args = [
        ...
        {},
        LabelFilter.empty(),
        JIRAFilter(1, LabelFilter({"performance"}, set()), set(), set(), False),
        branches, default_branches,
        False,
        release_match_setting_tag,
        mdb,
    ...
    args[7] = JIRAFilter(1, LabelFilter.empty(), {"DEV-149"}, set(), False)
    miner, _, _, _ = await PullRequestMiner.mine(*args)
    numbers = {pr.pr[PullRequest.number.key] for pr in miner}
    assert {821, 833, 846, 861} == numbers
    args[7] = JIRAFilter(1, LabelFilter.empty(), set(), {"bug"}, False)
    miner, _, _, _ = await PullRequestMiner.mine(*args)
    numbers = {pr.pr[PullRequest.number.key] for pr in miner}
    assert {800, 769, 896, 762, 807, 778, 855, 816, 754, 724, 790, 759, 792, 794, 795} == numbers
    args[7] = JIRAFilter(1, LabelFilter({"api"}, set()), set(), {"task"}, False)
    miner, _, _, _ = await PullRequestMiner.mine(*args)
    numbers = {pr.pr[PullRequest.number.key] for pr in miner}
    ...
    args[7] = JIRAFilter(1, LabelFilter.empty(), set(), set(), True)
    miner, _, _, _ = await PullRequestMiner.mine(*args)
    numbers = {pr.pr[PullRequest.number.key] for pr in miner}
...
async def test_pr_miner_jira_cache(
    ...
    await wait_deferred()
    assert len(miner) == 325
    args[7] = JIRAFilter(1, LabelFilter({"enhancement"}, set()), {"DEV-149"}, {"task"}, False)
    args[-3] = args[-2] = None
    miner, _, _, _ = await PullRequestMiner.mine(*args)
    assert len(miner) == 3
    for pr in miner:
    ...
    args[7] = JIRAFilter(
        1, LabelFilter({"enhancement,performance"}, set()), {"DEV-149"}, {"task"}, False)
    miner, _, _, _ = await PullRequestMiner.mine(*args)
    assert len(miner) == 0
    args[-3] = mdb
    args[-2] = pdb
