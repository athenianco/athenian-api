...
async def precomputed_merged_unreleased(
    mdb, pdb, rdb, dag, default_branches, release_match_setting_tag, release_loader, prefixer
):
...
async def test_store_merged_unreleased_pull_request_facts_smoke(
    ...
    dfs.prs.loc[
        dfs.prs[PullRequest.merged_at.name].isnull(), PullRequest.merged_at.name
