...
from athenian.api.controllers.miners.github.precomputed_prs import \
    ...
    load_precomputed_pr_releases, MergedPRFactsLoader, OpenPRFactsLoader, \
...
from athenian.api.experiments.preloading.entries import PreloadedMergedPRFactsLoader, \
    PreloadedOpenPRFactsLoader, PreloadedReleaseLoader
...
async def test_store_merged_unreleased_pull_request_facts_smoke(
    ...
    new_dict = {
        r[ghmprf.pr_node_id.key]: PullRequestFacts(
            data=r[ghmprf.data.key],
            repository_full_name="src-d/go-git",
            author=r[ghmprf.author.key],
            merger=r[ghmprf.merger.key])
        for r in rows
    }
...
async def test_store_open_pull_request_facts_smoke(
    ...
    for row in rows:
        ...
        new_dict[row[ghoprf.pr_node_id.key]] = PullRequestFacts(
