...
async def test_get_installation_id_conn(sdb):
    async with sdb.connection() as sdb_conn:
        assert await get_github_installation_ids(1, sdb_conn, None) == (6366825,)


async def test_get_installation_id_no_cache(sdb):
    assert await get_github_installation_ids(1, sdb, None) == (6366825,)


async def test_get_installation_id_error(sdb):
    with pytest.raises(ResponseError):
        await get_github_installation_ids(2, sdb, None)
