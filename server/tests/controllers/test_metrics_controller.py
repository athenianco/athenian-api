...
async def test_calc_metrics_prs_logical_smoke(
    client, headers, logical_settings_db, release_match_setting_tag_logical_db, second_repo, counts
):
    body = {
        "for": [
            {
...
@pytest.mark.parametrize(
    "metric, value", sorted((m, developer_metric_mcuadros_stats[m]) for m in DeveloperMetricID)
)
...
@pytest.mark.parametrize(
    "metric, value", list(developer_metric_mcuadros_jira_labels_stats.items())
)
...
@pytest.mark.parametrize(
    "metric, value", sorted((m, developer_metric_be_stats[m]) for m in DeveloperMetricID)
)
