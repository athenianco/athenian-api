...
from athenian.api.internal.features.histogram import Scale, calculate_histogram
...
def test_calculate_histogram_timedelta_linear_fixed():
    ...
    assert h.ticks == [
        ...
        timedelta(seconds=153),
        timedelta(seconds=247),
        timedelta(seconds=341),
        timedelta(seconds=435),
        timedelta(seconds=529),
        timedelta(seconds=623),
        timedelta(seconds=717),
        timedelta(seconds=811),
        timedelta(seconds=905),
...
def test_calculate_histogram_timedelta_linear_auto():
    ...
    assert h.ticks == [
        ...
        timedelta(seconds=216),
        timedelta(seconds=294),
        timedelta(seconds=373),
        timedelta(seconds=451),
        timedelta(seconds=529),
        timedelta(seconds=607),
        timedelta(seconds=686),
        timedelta(seconds=764),
        timedelta(seconds=842),
...
def test_calculate_histogram_timedelta_log_fixed():
    ...
    assert h.ticks[:-1] == [
        timedelta(seconds=60),
        timedelta(seconds=79),
        timedelta(seconds=105),
        timedelta(seconds=139),
        timedelta(seconds=184),
        timedelta(seconds=244),
        timedelta(seconds=324),
        timedelta(seconds=429),
        timedelta(seconds=569),
...
def test_calculate_histogram_timedelta_log_auto():
    ...
    assert h.ticks[:-1] == [
        ...
        timedelta(seconds=105),
        timedelta(seconds=127),
        timedelta(seconds=153),
        timedelta(seconds=184),
        timedelta(seconds=222),
        timedelta(seconds=268),
        timedelta(seconds=324),
        timedelta(seconds=391),
        timedelta(seconds=471),
        timedelta(seconds=569),
        timedelta(seconds=686),
        timedelta(seconds=828),
    ]
    # depends on the version of numpy
    assert h.ticks[-1] in (timedelta(seconds=999), timedelta(seconds=998))
...
def test_calculate_histogram_timedelta_ticks():
    h = calculate_histogram(
        ...
        None,
        None,
        [timedelta(seconds=240), timedelta(seconds=700)],
    )
    assert h.bins == 3
    assert h.scale == Scale.LINEAR
    assert h.ticks == [
