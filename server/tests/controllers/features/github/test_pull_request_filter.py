...
async def test_pr_list_miner_none(mdb, pdb, release_match_setting_tag, time_from_to):
    prs = await filter_pull_requests(
        set(), *time_from_to, {"src-d/go-git"}, {}, LabelFilter.empty(), JIRAFilter.empty(),
...
async def test_pr_list_miner_match_participants(mdb, pdb, release_match_setting_tag, time_from_to):
    ...
    prs = await filter_pull_requests(
        set(Property), *time_from_to, {"src-d/go-git"}, participants, LabelFilter.empty(),
...
async def test_pr_list_miner_match_metrics_all_count(
    ...
    prs = await filter_pull_requests(
        set(Property), time_from, time_to, {"src-d/go-git"}, {}, LabelFilter.empty(),
        JIRAFilter.empty(), False, release_match_setting_tag, 0, mdb, pdb, None)
    await wait_deferred()
    assert prs
...
async def test_pr_list_miner_release_settings(
    ...
    prs1 = await filter_pull_requests(
        {Property.RELEASING}, time_from, time_to, {"src-d/go-git"}, {}, LabelFilter.empty(),
        JIRAFilter.empty(), False, release_match_setting_tag, 0, mdb, pdb, cache)
    assert prs1
    await wait_deferred()
    prs2 = await filter_pull_requests(
        {Property.RELEASING}, time_from, time_to, {"src-d/go-git"}, {}, LabelFilter.empty(),
        JIRAFilter.empty(), False, release_match_setting_branch, 0, mdb, pdb, cache)
...
async def test_pr_list_miner_limit(
    ...
    prs = await filter_pull_requests(
        {Property.RELEASING}, time_from, time_to, {"src-d/go-git"}, {}, LabelFilter.empty(),
        JIRAFilter.empty(), False, release_match_setting_branch, 10, mdb, pdb, cache)
    assert len(prs) == 7
...
async def test_pr_list_miner_exclude_inactive(mdb, pdb, release_match_setting_tag, cache):
    ...
    prs1 = await filter_pull_requests(
        set(Property), time_from, time_to, {"src-d/go-git"}, {}, LabelFilter.empty(),
        JIRAFilter.empty(), True, release_match_setting_tag, 0, mdb, pdb, cache)
    assert len(prs1) == 6
...
async def test_pr_list_miner_filter_labels_cache_postprocess(
    ...
    with pytest.raises(Exception):
        await filter_pull_requests(
            set(Property), time_from, time_to, {"src-d/go-git"}, {}, LabelFilter.empty(),
            JIRAFilter.empty(), False, release_match_setting_tag, 0, None, None, cache)
