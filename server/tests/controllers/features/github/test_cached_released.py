...
async def test_load_store_cached_released_times(
        cache, pr_samples, size, release_match_setting_tag):
    ...
    names = ["one", "two", "three"]
    settings = {"github.com/" + k: next(iter(release_match_setting_tag.values())) for k in names}
    ...
    await store_cached_released_times(prs, settings, cache)
    ...
    loaded_prs = await load_cached_released_times(
        date_from, date_to, names, [], settings, cache)
...
async def test_load_store_cached_released_filters(pr_samples, cache, release_match_setting_tag):
    ...
    loaded_prs = await load_cached_released_times(
        date_from, date_to, ["one"], ["xxx"], settings, cache)
    for k, load_value in loaded_prs.items():
        pr = prs_by_key[k]
    ...
    for k, s in settings.items():
        settings[k] = ReleaseMatchSetting(s.branches, s.tags, Match.branch)
    loaded_prs = await load_cached_released_times(
        date_from, date_to, ["one"], ["xxx"], settings, cache)
