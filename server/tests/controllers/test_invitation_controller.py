...
async def test_accept_invitation_user_profile(
    ...
    try:
        ...
        await app._auth0.update_user_profile(
            "auth0|62a1ae88b6bba16c6dbc6870", name="Vadim Markovtsev", email="vadim@athenian.co",
        )
...
async def test_accept_invitation_banished(client, headers, sdb, disable_default_user, app):
    app._auth0._default_user = app._auth0._default_user.copy()
    app._auth0._default_user.login = "vmarkovtsev"
    check_fid = await sdb.fetch_val(
        select([Feature.id]).where(
...
async def test_accept_invitation_github_disabled(
    ...
    if as_github:
        ...
        await sdb.execute(
            update(Feature)
            .where(Feature.name == Feature.USER_ORG_MEMBERSHIP_CHECK)
            .values(
                {
    ...
    await sdb.execute(
        update(Feature)
        .where(Feature.name == Feature.GITHUB_LOGIN_ENABLED)
        .values(
            {
                Feature.enabled: False,
                Feature.updated_at: datetime.now(timezone.utc),
...
async def test_jira_progress_not_finished(client, headers, mdb_rw):
    await mdb_rw.execute(update(JIRAProgress).values({JIRAProgress.total: JIRAProgress.total * 2}))
    try:
        response = await client.request(
            method="GET", path="/v1/invite/jira_progress/1", headers=headers, json={},
        )
        ...
        await mdb_rw.execute(
            update(JIRAProgress).values({JIRAProgress.total: JIRAProgress.total / 2}),
        )
