...
@with_defer
async def test_get_calculator_for_user_cached(sdb, mdb, pdb, rdb, cache, base_testing_module):
    feature_name = f"{METRIC_ENTRIES_VARIATIONS_PREFIX['github']}test_calculator_selector"
    feature_id = await sdb.execute(
        insert(Feature).values(
            Feature(
                name=feature_name,
    ...
    await sdb.execute(update(Feature).where(
        Feature.name == feature_name).values({
    ...
    calc = (await get_calculators_for_account(
        ["github"], 1, (1, ), None, sdb, mdb, pdb, rdb, None,
        base_module=base_testing_module,
    ))["github"]
    assert isinstance(calc, MetricEntriesCalculator)
