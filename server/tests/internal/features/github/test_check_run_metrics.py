...
def alternative_facts(alternative_check_run_facts):
    return _finalize_check_runs(
        alternative_check_run_facts, logging.getLogger("pytest.alternative_facts"),
    )
...
async def test_check_run_metrics_robust_quantiles(alternative_facts):
    ...
    cls(
        alternative_facts,
        np.array([datetime(2021, 1, 1), datetime(2021, 6, 1)], dtype="datetime64[us]"),
        np.array([datetime(2021, 6, 1), datetime(2021, 11, 1)], dtype="datetime64[us]"),
...
def test_elapsed_time_per_concurrency_histogram():
    ...
    facts = md.DataFrame(
        {
            CheckRun.started_at.name: ii_starts.view("datetime64[s]").astype("datetime64[us]"),
            CheckRun.completed_at.name: ii_finishes.view("datetime64[s]").astype("datetime64[us]"),
            CheckRun.repository_full_name.name: ["athenianco/athenian-api"] * len(ii_starts),
            CheckRun.name.name: np.array(["name1"] * 6 + ["name2"] * 7, dtype=object),
        },
    )
    calc(
        facts,
        np.array([0], dtype="datetime64[s]").astype("datetime64[us]"),
        np.array([500], dtype="datetime64[s]").astype("datetime64[us]"),
        [np.arange(len(facts))],
    )
...
def test_check_run_metrics_alternative_blitz(alternative_facts, metric, value):
    ...
    cls(
        alternative_facts,
        np.array([datetime(2021, 1, 1)], dtype="datetime64[us]"),
        np.array([datetime(2021, 11, 1)], dtype="datetime64[us]"),
        [np.arange(len(alternative_facts))],
    )
