...
class TestLogicalRepositorySettings:
    def test_explode_user_repos(self) -> None:
        ...
        exploded = settings.explode_user_repos(["src-d/go-git", "src-d/foobar"])
        ...
        exploded = settings.explode_user_repos(["src-d/go-git"])
        assert exploded[0] == "src-d/go-git"
        assert sorted(exploded[1:3]) == ["src-d/go-git/alpha", "src-d/go-git/beta"]

        exploded = settings.explode_user_repos(["src-d/go-git", "src-d/go-git/alpha"])
        assert exploded == ["src-d/go-git", "src-d/go-git/alpha"]

        exploded = settings.explode_user_repos(["src-d/go-git/alpha"])
